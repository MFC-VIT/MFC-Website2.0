...
const Projects = () => {
  ...
  const projectDetailElementRefs = React.useMemo(() => [
    previewImageRef, previewNumberRef, previewTitleRef,
    previewDescRef, previewTagsRef, previewLinkRef,
  ], []);
  const foxFallbackElementRefs = React.useMemo(() => [
    foxImageRef, foxTextLine1Ref, foxTextLine2Ref
  ], []);

  const getNonNullRefs = (refsArray) => refsArray.map(ref => ref.current).filter(Boolean);
  ...
  useEffect(() => {
    projectListRefs.current.forEach((el) => {
      ...
      if (isSelected || isHovered) {
        ...
        gsap.to(number, { color: "#ff6d00", duration: 0.3, ease: "power3.out" }); // brand-orange
        gsap.to(name, { color: isSelected ? "#ff9a00" : "#ffeadb", duration: 0.3, ease: "power3.out" }); // brand-orange-light
        el.classList.add("is-active-item");
      } else {
        gsap.to(content, { x: 0, duration: 0.3, ease: "power3.out" });
        gsap.to(number, { color: "#a39e9a", duration: 0.3, ease: "power3.out" });
    ...
    if (activeProject) {
      if (currentProjectDetails.length > 0) {
        gsap.to(currentProjectDetails, {
          autoAlpha: 0, y: 20, scale: 0.98, duration: 0.25, stagger: 0.03, ease: "power2.in",
  ...
  useEffect(() => {
    ...
    if (activeProject) {
      ...
      if (previewNumberRef.current) previewNumberRef.current.textContent = activeProject.projectNumber;
      if (previewTitleRef.current) previewTitleRef.current.textContent = activeProject.projectName;
      if (previewDescRef.current) previewDescRef.current.textContent = activeProject.description;

      if (previewImageRef.current && activeProject.projectImage) {
        const img = new Image();
        img.src = activeProject.projectImage;
        img.onload = () => {
          gsap.set(previewImageRef.current, { backgroundImage: `url(${activeProject.projectImage})` });
          if (currentProjectDetails.length > 0) {
            gsap.fromTo(currentProjectDetails,
              { autoAlpha: 0, y: -20, scale: 1.02 },
              { autoAlpha: 1, y: 0, scale: 1, duration: 0.4, stagger: 0.05, ease: "power3.out", delay: 0.05 }
            );
          }
        };
        img.onerror = () => {
          gsap.set(previewImageRef.current, { backgroundImage: `url(${FoxFallbackImage})` });
          const otherDetails = currentProjectDetails.filter(el => el !== previewImageRef.current);
          if (otherDetails.length > 0) {
            gsap.fromTo(otherDetails,
              { autoAlpha: 0, y: -20, scale: 1.02 },
              { autoAlpha: 1, y: 0, scale: 1, duration: 0.4, stagger: 0.05, ease: "power3.out", delay: 0.05 }
            );
          }
  ...
  useEffect(() => {
    const allElements = [...getNonNullRefs(projectDetailElementRefs), ...getNonNullRefs(foxFallbackElementRefs)];
  ...
  return (
    <div id="projects" className="min-h-screen h-fit w-full flex flex-col pt-24 md:pt-32 pb-20 md:pb-28 relative bg-black text-main">
        ...
        <div className="flex flex-col md:flex-row gap-8 lg:gap-12">
            ...
            {projects.map((item) => (
              ...
              >
                <div className={`absolute top-0 left-0 h-full w-1 
                                ...
                                ${selectedProjectId === item.id ? '!opacity-100 !via-orange-500' : ''}`}>
                </div>
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5 text-secondary/50 self-center transition-transform duration-300 group-hover:translate-x-1 group-[.is-active-item]:text-orange-400">
          ...
          <div className="w-full md:w-[55%] lg:w-[60%] md:sticky md:top-24 h-[60vh] md:h-auto md:max-h-[70vh] mt-8 md:mt-0">
            ...
            >
              ...
              <div ref={previewImageRef} className="w-full h-48 sm:h-64 md:h-1/2 lg:h-[55%] bg-cover bg-center rounded-lg mb-4 md:mb-6 shadow-lg border border-neutral-700"></div>
              <div className="flex-grow flex flex-col">
                <p ref={previewNumberRef} className="font-yoshiro text-5xl md:text-7xl text-orange-500/40 absolute top-5 right-7 md:top-7 md:right-9"></p>
                <h2 ref={previewTitleRef} className="font-apex text-2xl md:text-3xl lg:text-4xl text-main mb-2"></h2>
                <p ref={previewDescRef} className="text-secondary text-sm md:text-base leading-relaxed mb-3 md:mb-4 flex-grow"></p>
                ...
                {activeProject && (
                    ...
                    >
                    View Project
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-4 h-4 ml-2 transform transition-transform duration-300 group-hover:translate-x-1">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center pointer-events-none">
                ...
                />
                <p ref={foxTextLine1Ref} className="font-apex text-2xl text-secondary/60">Hover or Click a project</p>
                <p ref={foxTextLine2Ref} className="text-secondary/50">to see details here.</p>
              </div>
            </div>
          </div>
        </div>
...
const projects = [
    ...
    { id: 3, projectNumber: "03", projectName: "Tech Wars", tagline: "Engaging platform for competitive tech events.", description: "Mozilla VIT's flagship event website with schedules, registration, and real-time updates. Designed for high traffic.", projectLink: "https://www.mozillavit.in/", projectImage: "https://images.unsplash.com/photo-1519681393784-d120267933ba?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8Nnx8dGVjaCUyMGJhY2tncm91bmR8ZW58MHx8MHx8fDA%3D&auto=format&fit=crop&w=1200&q=80", tags: ["React", "Gatsby", "CMS", "Events"]},
    { id: 4, projectNumber: "04", projectName: "Parallax Visions", tagline: "Immersive scrolling experiences.", description: "An exploration of parallax scrolling for dynamic web narratives, showcasing advanced CSS and JavaScript for visual storytelling.", projectLink: "https://www.mozillavit.in/", projectImage: "https://images.unsplash.com/photo-1487088678257-3a541e6e3922?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8MTB8fHBhcmFsbGF4fGVufDB8fDB8fHww&auto=format&fit=crop&w=1200&q=80", tags: ["JavaScript", "GSAP", "UI/UX", "Web Design"]},
    { id: 5, projectNumber: "05", projectName: "Future Scapes", tagline: "Conceptualizing tomorrow's digital interfaces.", description: "A design-driven project on futuristic UI/UX concepts, featuring interactive prototypes that push interface boundaries.", projectLink: "https://www.mozillavit.in/", projectImage: "https://images.unsplash.com/photo-1507608443049-a23a48acos36e?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxzZWFyY2h8NHx8ZnV0dXJpc3RpYyUyMGRlc2lnbnxlbnwwfHwwfHx8MA%3D%3D&auto=format&fit=crop&w=1200&q=80", tags: ["UI Design", "UX Research", "Prototyping", "Figma"]},
];