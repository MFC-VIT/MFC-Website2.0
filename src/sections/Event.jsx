...
const Event = ({ title, description, doe, image, link }) => {
  return (
    <div className="flex max-sm:flex-col justify-between border-[3px] border-primary rounded-2xl p-4 min-h-64 gap-5 max-sm:gap-1 bg-black">
...
const Link = ({ link, text }) => {
  return (
    ...
    >
      <span className="text-main text-sm max-sm:text-xs tracking-wider">
        {text}
      </span>
...
const SeeMore = ({ link }) => {
  return (
    <a
      ...
      className="w-fit bg-[#0A0807] border-2 border-[#F7813F] hover:bg-[#F7813F] hover:text-white py-2 px-3 sm:px-4 rounded-full flex items-center font-apex gap-1 sm:gap-2 group"
    >
      <span className="text-main text-base max-sm:text-sm tracking-wider">
        {"See More"}
      </span>
      <BsArrowUpRightCircle className="text-base text-[#F7813F] group-hover:text-white" />
    </a>
